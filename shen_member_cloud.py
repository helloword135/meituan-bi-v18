
# V20.1 神会员批量上传版：多文件上传、逐个预计算、页面读取汇总表
import re
from io import BytesIO
import pandas as pd
import streamlit as st

SHEN_TABLE = "shen_member_history"
SHOP_MASTER_TABLE = "shop_master"
SHEN_DAILY_SUMMARY_TABLE = "shen_daily_summary"
SHEN_BD_DAILY_SUMMARY_TABLE = "shen_bd_daily_summary"

REQUIRED_SHEN_COLS = ["门店名称", "是否报名", "是否重点门店", "是否膨胀报名"]
REQUIRED_BD_COLS = ["门店名称", "BD"]


def _normalize_flag(series):
    return (
        series.fillna(0).astype(str).str.strip().replace({
            "1": "1", "1.0": "1", "是": "1", "TRUE": "1", "True": "1", "true": "1",
            "已报名": "1", "已参与": "1", "Y": "1", "y": "1",
            "0": "0", "0.0": "0", "否": "0", "FALSE": "0", "False": "0", "false": "0",
            "未报名": "0", "未参与": "0", "N": "0", "n": "0",
        }).apply(lambda x: 1 if x == "1" else 0)
    )


def _date_from_filename(filename):
    name = filename or ""
    m = re.search(r"(20\d{2})[-_/\.]?(0?\d|1[0-2])[-_/\.]?([0-3]?\d)", name)
    if not m:
        return None
    y, mth, d = m.group(1), m.group(2).zfill(2), m.group(3).zfill(2)
    try:
        return pd.to_datetime(f"{y}-{mth}-{d}").strftime("%Y-%m-%d")
    except Exception:
        return None


def validate_shen_df(df):
    missing = [c for c in REQUIRED_SHEN_COLS if c not in df.columns]
    if missing:
        raise Exception("神会员数据缺少字段：" + "、".join(missing))


def validate_bd_df(df):
    missing = [c for c in REQUIRED_BD_COLS if c not in df.columns]
    if missing:
        raise Exception("BD门店明细缺少字段：" + "、".join(missing))


def prepare_shen_df(df, filename=None):
    validate_shen_df(df)
    df = df.copy()
    df["门店名称"] = df["门店名称"].astype(str).str.strip()

    if "快照日期" not in df.columns:
        business_date = _date_from_filename(filename)
        if business_date is None:
            raise Exception("神会员数据缺少【快照日期】字段，且文件名无法识别日期。请把文件名改成 2026-05-18.xlsx 这种格式。")
        df["快照日期"] = business_date
    else:
        df["快照日期"] = pd.to_datetime(df["快照日期"], errors="coerce").dt.strftime("%Y-%m-%d")

    df = df[df["快照日期"].notna()]
    for col in ["是否报名", "是否重点门店", "是否膨胀报名"]:
        df[col] = _normalize_flag(df[col])

    if "来源文件" not in df.columns:
        df["来源文件"] = filename or ""

    return df


def prepare_bd_df(df):
    validate_bd_df(df)
    df = df.copy()
    df["门店名称"] = df["门店名称"].astype(str).str.strip()
    df["BD"] = df["BD"].astype(str).str.strip()
    df = df[df["门店名称"].notna()]
    df = df[df["门店名称"] != ""]
    df = df[df["BD"].notna()]
    df = df[df["BD"] != ""]
    df = df[df["BD"].str.lower() != "nan"]
    return df.drop_duplicates(subset=["门店名称"], keep="last")


def _row_to_json(row, columns):
    data_json = {}
    for col in columns:
        val = row[col]
        if pd.isna(val):
            data_json[col] = None
        elif isinstance(val, pd.Timestamp):
            data_json[col] = val.strftime("%Y-%m-%d")
        else:
            data_json[col] = val.item() if hasattr(val, "item") else val
    return data_json


def calc_shen_city_report(shen_df):
    df = prepare_shen_df(shen_df)
    result_list = []
    for business_date in sorted(df["快照日期"].drop_duplicates()):
        day_df = df[df["快照日期"] == business_date]
        deal_total = len(day_df)
        deal_signup = len(day_df[day_df["是否报名"] == 1])
        deal_rate = round(deal_signup / deal_total * 100, 2) if deal_total > 0 else 0
        expand_df = day_df[day_df["是否重点门店"] == 1]
        expand_total = len(expand_df)
        expand_signup = len(day_df[(day_df["是否重点门店"] == 1) & (day_df["是否膨胀报名"] == 1)])
        expand_rate = round(expand_signup / expand_total * 100, 2) if expand_total > 0 else 0
        result_list.append({
            "日期": business_date,
            "deal总数": deal_total,
            "deal报名数": deal_signup,
            "deal覆盖率": deal_rate,
            "膨胀deal总数": expand_total,
            "膨胀deal报名数": expand_signup,
            "膨胀deal覆盖率": expand_rate,
        })
    return pd.DataFrame(result_list)


def calc_shen_bd_report(shen_df, bd_mapping_df):
    shen_df = prepare_shen_df(shen_df)
    if bd_mapping_df.empty:
        return pd.DataFrame(), shen_df.copy()

    bd_df = prepare_bd_df(bd_mapping_df)
    merge_df = shen_df.merge(bd_df[["门店名称", "BD"]], on="门店名称", how="left")

    unmatched_df = merge_df[
        merge_df["BD"].isna()
        | (merge_df["BD"].astype(str).str.strip() == "")
        | (merge_df["BD"].astype(str).str.lower() == "nan")
    ].copy()

    merge_df = merge_df[merge_df["BD"].notna()]
    merge_df = merge_df[merge_df["BD"].astype(str).str.strip() != ""]
    merge_df = merge_df[merge_df["BD"].astype(str).str.lower() != "nan"]

    result_list = []
    for business_date in sorted(merge_df["快照日期"].drop_duplicates()):
        day_df = merge_df[merge_df["快照日期"] == business_date]
        for bd_name in sorted(day_df["BD"].drop_duplicates()):
            bd_day_df = day_df[day_df["BD"] == bd_name]
            deal_total = len(bd_day_df)
            deal_signup = len(bd_day_df[bd_day_df["是否报名"] == 1])
            deal_rate = round(deal_signup / deal_total * 100, 2) if deal_total > 0 else 0
            expand_df = bd_day_df[bd_day_df["是否重点门店"] == 1]
            expand_total = len(expand_df)
            expand_signup = len(bd_day_df[(bd_day_df["是否重点门店"] == 1) & (bd_day_df["是否膨胀报名"] == 1)])
            expand_rate = round(expand_signup / expand_total * 100, 2) if expand_total > 0 else 0
            result_list.append({
                "日期": business_date,
                "当前私海BD": bd_name,
                "deal总数": deal_total,
                "deal报名数": deal_signup,
                "deal覆盖率": deal_rate,
                "膨胀deal总数": expand_total,
                "膨胀deal报名数": expand_signup,
                "膨胀deal覆盖率": expand_rate,
            })
    return pd.DataFrame(result_list), unmatched_df


def load_shop_master_from_cloud(client, project_code):
    rows = []
    start = 0
    page_size = 1000
    while True:
        resp = (
            client.table(SHOP_MASTER_TABLE)
            .select("shop_name,bd,data")
            .eq("project_code", project_code)
            .range(start, start + page_size - 1)
            .execute()
        )
        batch = resp.data or []
        if not batch:
            break
        rows.extend(batch)
        if len(batch) < page_size:
            break
        start += page_size

    if not rows:
        return pd.DataFrame(columns=["门店名称", "BD"])

    result = []
    for r in rows:
        data = r.get("data") or {}
        result.append({
            "门店名称": r.get("shop_name") or data.get("门店名称"),
            "BD": r.get("bd") or data.get("BD")
        })
    return prepare_bd_df(pd.DataFrame(result))


def get_shop_master_status(client, project_code):
    df = load_shop_master_from_cloud(client, project_code)
    if df.empty:
        return {"项目编码": project_code, "BD档案库状态": "暂无BD档案"}
    return {"项目编码": project_code, "BD档案门店数": len(df), "BD人数": df["BD"].nunique()}


def upsert_shop_master_to_cloud(client, project_code, uploaded_file):
    df_raw = pd.read_excel(uploaded_file)
    df = prepare_bd_df(df_raw)

    client.table(SHOP_MASTER_TABLE).delete().eq("project_code", project_code).execute()

    records = []
    for _, row in df.iterrows():
        records.append({
            "project_code": project_code,
            "shop_name": str(row["门店名称"]),
            "bd": str(row["BD"]),
            "data": _row_to_json(row, df.columns),
        })
    for i in range(0, len(records), 500):
        client.table(SHOP_MASTER_TABLE).insert(records[i:i + 500]).execute()
    return len(records)


def delete_shop_master(client, project_code):
    client.table(SHOP_MASTER_TABLE).delete().eq("project_code", project_code).execute()


def upsert_shen_daily_summary(client, project_code, city_report):
    if city_report.empty:
        return
    for _, row in city_report.iterrows():
        record = {
            "project_code": project_code,
            "summary_date": row["日期"],
            "deal_total": int(row["deal总数"]),
            "deal_signup": int(row["deal报名数"]),
            "deal_rate": float(row["deal覆盖率"]),
            "expand_total": int(row["膨胀deal总数"]),
            "expand_signup": int(row["膨胀deal报名数"]),
            "expand_rate": float(row["膨胀deal覆盖率"]),
            "data": dict(row),
        }
        client.table(SHEN_DAILY_SUMMARY_TABLE).upsert(record, on_conflict="project_code,summary_date").execute()


def upsert_shen_bd_daily_summary(client, project_code, bd_report):
    if bd_report is None or bd_report.empty:
        return
    date_list = sorted(bd_report["日期"].dropna().unique().tolist())
    for d in date_list:
        client.table(SHEN_BD_DAILY_SUMMARY_TABLE).delete().eq("project_code", project_code).eq("summary_date", d).execute()

    records = []
    for _, row in bd_report.iterrows():
        records.append({
            "project_code": project_code,
            "summary_date": row["日期"],
            "bd": row["当前私海BD"],
            "deal_total": int(row["deal总数"]),
            "deal_signup": int(row["deal报名数"]),
            "deal_rate": float(row["deal覆盖率"]),
            "expand_total": int(row["膨胀deal总数"]),
            "expand_signup": int(row["膨胀deal报名数"]),
            "expand_rate": float(row["膨胀deal覆盖率"]),
            "data": dict(row),
        })
    for i in range(0, len(records), 500):
        client.table(SHEN_BD_DAILY_SUMMARY_TABLE).insert(records[i:i + 500]).execute()


def upsert_shen_member_to_cloud(client, project_code, uploaded_file):
    df_raw = pd.read_excel(uploaded_file)
    df = prepare_shen_df(df_raw, filename=getattr(uploaded_file, "name", ""))
    date_list = sorted(df["快照日期"].dropna().unique().tolist())

    # 明细保留，方便下载和追溯；同日覆盖
    for date_text in date_list:
        client.table(SHEN_TABLE).delete().eq("project_code", project_code).eq("snapshot_date", date_text).execute()

    records = []
    for _, row in df.iterrows():
        records.append({
            "project_code": project_code,
            "snapshot_date": row["快照日期"],
            "shop_name": str(row.get("门店名称", "")),
            "data": _row_to_json(row, df.columns),
        })
    for i in range(0, len(records), 500):
        client.table(SHEN_TABLE).insert(records[i:i + 500]).execute()

    # 上传时预计算
    city_report = calc_shen_city_report(df)
    upsert_shen_daily_summary(client, project_code, city_report)

    bd_mapping = load_shop_master_from_cloud(client, project_code)
    bd_report, unmatched_df = calc_shen_bd_report(df, bd_mapping)
    upsert_shen_bd_daily_summary(client, project_code, bd_report)

    return len(records), date_list, city_report, bd_report, unmatched_df


def get_shen_member_status(client, project_code):
    resp = (
        client.table(SHEN_DAILY_SUMMARY_TABLE)
        .select("summary_date,deal_total")
        .eq("project_code", project_code)
        .order("summary_date")
        .execute()
    )
    rows = resp.data or []
    if not rows:
        return {"项目编码": project_code, "神会员历史库状态": "暂无神会员数据"}
    df = pd.DataFrame(rows)
    return {
        "项目编码": project_code,
        "已汇总日期数": len(df),
        "最早日期": str(pd.to_datetime(df["summary_date"]).min().date()),
        "最新日期": str(pd.to_datetime(df["summary_date"]).max().date()),
    }


def get_shen_uploaded_dates(client, project_code):
    resp = (
        client.table(SHEN_DAILY_SUMMARY_TABLE)
        .select("summary_date,deal_total")
        .eq("project_code", project_code)
        .order("summary_date")
        .execute()
    )
    rows = resp.data or []
    if not rows:
        return pd.DataFrame(columns=["日期", "记录数", "门店数"])
    df = pd.DataFrame(rows)
    return df.rename(columns={"summary_date": "日期", "deal_total": "记录数"}).assign(门店数=lambda x: x["记录数"])


def load_city_summary(client, project_code, start_date=None, end_date=None):
    query = client.table(SHEN_DAILY_SUMMARY_TABLE).select("*").eq("project_code", project_code)
    if start_date:
        query = query.gte("summary_date", str(start_date))
    if end_date:
        query = query.lte("summary_date", str(end_date))
    resp = query.order("summary_date").execute()
    rows = resp.data or []
    if not rows:
        return pd.DataFrame()
    return pd.DataFrame([{
        "日期": r["summary_date"],
        "deal总数": r.get("deal_total", 0),
        "deal报名数": r.get("deal_signup", 0),
        "deal覆盖率": r.get("deal_rate", 0),
        "膨胀deal总数": r.get("expand_total", 0),
        "膨胀deal报名数": r.get("expand_signup", 0),
        "膨胀deal覆盖率": r.get("expand_rate", 0),
    } for r in rows])


def load_bd_summary(client, project_code, start_date=None, end_date=None):
    query = client.table(SHEN_BD_DAILY_SUMMARY_TABLE).select("*").eq("project_code", project_code)
    if start_date:
        query = query.gte("summary_date", str(start_date))
    if end_date:
        query = query.lte("summary_date", str(end_date))
    resp = query.order("summary_date").execute()
    rows = resp.data or []
    if not rows:
        return pd.DataFrame()
    return pd.DataFrame([{
        "日期": r["summary_date"],
        "当前私海BD": r.get("bd", ""),
        "deal总数": r.get("deal_total", 0),
        "deal报名数": r.get("deal_signup", 0),
        "deal覆盖率": r.get("deal_rate", 0),
        "膨胀deal总数": r.get("expand_total", 0),
        "膨胀deal报名数": r.get("expand_signup", 0),
        "膨胀deal覆盖率": r.get("expand_rate", 0),
    } for r in rows])


def delete_shen_member_by_date(client, project_code, snapshot_date):
    d = str(snapshot_date)
    client.table(SHEN_TABLE).delete().eq("project_code", project_code).eq("snapshot_date", d).execute()
    client.table(SHEN_DAILY_SUMMARY_TABLE).delete().eq("project_code", project_code).eq("summary_date", d).execute()
    client.table(SHEN_BD_DAILY_SUMMARY_TABLE).delete().eq("project_code", project_code).eq("summary_date", d).execute()


def delete_shen_member_data(client, project_code):
    client.table(SHEN_TABLE).delete().eq("project_code", project_code).execute()
    client.table(SHEN_DAILY_SUMMARY_TABLE).delete().eq("project_code", project_code).execute()
    client.table(SHEN_BD_DAILY_SUMMARY_TABLE).delete().eq("project_code", project_code).execute()


def _make_excel_bytes(city_report, bd_report=None):
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        city_report.to_excel(writer, index=False, sheet_name="神会员日报")
        if bd_report is not None and not bd_report.empty:
            bd_report.to_excel(writer, index=False, sheet_name="神会员BD日报")
    output.seek(0)
    return output.getvalue()


def render_shen_member_cloud_monitor(client, project_code):
    st.subheader("神会员数据监控")
    st.info("V20极速版：上传时预计算日报和BD日报；页面只读取汇总表，速度更快。")

    st.markdown("### 一、云历史库状态")
    c1, c2 = st.columns(2)
    with c1:
        st.write("神会员汇总库")
        try:
            st.write(get_shen_member_status(client, project_code))
            if st.button("查看已上传日期", key="show_shen_uploaded_dates"):
                date_df = get_shen_uploaded_dates(client, project_code)
                st.dataframe(date_df, use_container_width=True)
        except Exception as e:
            st.error("读取神会员汇总库失败，请确认已执行 V20 SQL。")
            st.exception(e)
            return

    with c2:
        st.write("BD档案库")
        try:
            st.write(get_shop_master_status(client, project_code))
        except Exception as e:
            st.error("读取BD档案库失败，请确认已执行 V20 SQL。")
            st.exception(e)

    st.divider()
    st.markdown("### 二、上传/更新BD门店档案")
    st.info("BD门店无变更，可不上传。仅首次使用或BD调整时上传。")

    bd_file = st.file_uploader("上传BD门店明细.xlsx", type=["xlsx", "xls"], key="bd_master_file")
    if st.button("保存/更新BD档案库", type="primary"):
        if bd_file is None:
            st.error("请先上传BD门店明细.xlsx。")
        else:
            try:
                count = upsert_shop_master_to_cloud(client, project_code, bd_file)
                st.success(f"BD档案库保存成功，共 {count} 家门店。")
                st.write(get_shop_master_status(client, project_code))
            except Exception as e:
                st.error("BD档案库保存失败。")
                st.exception(e)

    st.divider()
    st.markdown("### 三、批量上传神会员每日数据")
    shen_files = st.file_uploader(
        "上传神会员Excel（支持多文件批量上传）",
        type=["xlsx", "xls"],
        key="shen_member_cloud_files",
        accept_multiple_files=True
    )

    if st.button("批量保存神会员数据到云历史库", type="primary"):
        if not shen_files:
            st.error("请先上传一个或多个神会员Excel。")
        else:
            success_rows = []
            fail_rows = []
            total_count = 0
            total_unmatched = 0

            progress = st.progress(0)
            status_text = st.empty()

            for idx, file in enumerate(shen_files, start=1):
                try:
                    status_text.write(f"正在处理：{file.name}（{idx}/{len(shen_files)}）")

                    count, date_list, city_report, bd_report, unmatched_df = upsert_shen_member_to_cloud(
                        client,
                        project_code,
                        file
                    )

                    unmatched_count = 0
                    if unmatched_df is not None and not unmatched_df.empty:
                        unmatched_count = len(unmatched_df)

                    total_count += count
                    total_unmatched += unmatched_count

                    success_rows.append({
                        "文件名": file.name,
                        "处理行数": count,
                        "识别日期": "、".join(date_list),
                        "未匹配BD行数": unmatched_count,
                        "状态": "成功"
                    })

                except Exception as e:
                    fail_rows.append({
                        "文件名": file.name,
                        "错误原因": str(e),
                        "状态": "失败"
                    })

                progress.progress(idx / len(shen_files))

            status_text.write("批量处理完成。")

            if success_rows:
                st.success(f"成功处理 {len(success_rows)} 个文件，共 {total_count} 行。")
                st.dataframe(pd.DataFrame(success_rows), use_container_width=True)

            if fail_rows:
                st.error(f"失败 {len(fail_rows)} 个文件。")
                st.dataframe(pd.DataFrame(fail_rows), use_container_width=True)

            if total_unmatched > 0:
                st.warning(f"本次共有 {total_unmatched} 行神会员数据未匹配到BD。")

            try:
                st.write(get_shen_member_status(client, project_code))
            except Exception:
                pass

    with st.expander("神会员数据删除"):
        st.info("建议按日期删除，避免全量删除超时。")
        try:
            date_df = get_shen_uploaded_dates(client, project_code)
            if date_df.empty:
                st.warning("当前没有可删除的神会员日期。")
            else:
                st.dataframe(date_df, use_container_width=True)
                delete_date = st.selectbox("选择要删除的日期", date_df["日期"].tolist(), key="delete_shen_date_select")
                confirm_delete_date = st.checkbox("我确认删除所选日期的神会员数据")
                if confirm_delete_date and st.button("删除所选日期神会员数据", type="primary"):
                    delete_shen_member_by_date(client, project_code, delete_date)
                    st.success(f"已删除 {project_code} / {delete_date} 的神会员数据。")
        except Exception as e:
            st.error("读取已上传日期失败。")
            st.exception(e)

    st.divider()
    st.markdown("### 四、神会员历史监控")

    date_df = get_shen_uploaded_dates(client, project_code)
    if date_df.empty:
        st.warning("当前项目暂无神会员汇总数据。")
        return

    min_date = pd.to_datetime(date_df["日期"]).min().date()
    max_date = pd.to_datetime(date_df["日期"]).max().date()

    col_start, col_end = st.columns(2)
    start_date = col_start.date_input("神会员开始日期", value=min_date, min_value=min_date, max_value=max_date, key="shen_start_date")
    end_date = col_end.date_input("神会员结束日期", value=max_date, min_value=min_date, max_value=max_date, key="shen_end_date")

    city_report = load_city_summary(client, project_code, start_date, end_date)
    if city_report.empty:
        st.warning("当前日期范围内没有神会员汇总数据。")
        return

    st.markdown("### 五、神会员日报")
    st.dataframe(city_report.tail(31), use_container_width=True)
    st.caption(f"仅展示最近31行，共 {len(city_report)} 行。")

    latest = city_report.iloc[-1]
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("最新deal总数", int(latest["deal总数"]))
    c2.metric("最新deal覆盖率", f'{latest["deal覆盖率"]}%')
    c3.metric("最新膨胀deal总数", int(latest["膨胀deal总数"]))
    c4.metric("最新膨胀覆盖率", f'{latest["膨胀deal覆盖率"]}%')

    st.markdown("### 六、覆盖率趋势")
    trend_df = city_report.set_index("日期")[["deal覆盖率", "膨胀deal覆盖率"]]
    st.line_chart(trend_df, use_container_width=True)

    st.divider()
    st.markdown("### 七、神会员BD日报")

    bd_report = load_bd_summary(client, project_code, start_date, end_date)
    if bd_report.empty:
        st.warning("当前日期范围内暂无BD汇总数据。请确认已上传BD档案，并重新上传/覆盖神会员数据以生成BD汇总。")
    else:
        st.dataframe(bd_report.head(500), use_container_width=True)
        st.caption(f"仅预览前500行，共 {len(bd_report)} 行。")

        latest_date = bd_report["日期"].max()
        latest_bd = bd_report[bd_report["日期"] == latest_date].copy()
        st.markdown(f"### 八、BD覆盖率排行（{latest_date}）")
        rank_df = latest_bd.sort_values(by=["deal覆盖率", "膨胀deal覆盖率"], ascending=False)
        st.dataframe(rank_df, use_container_width=True)

        st.markdown("### 九、BD覆盖率图")
        chart_df = rank_df.set_index("当前私海BD")[["deal覆盖率", "膨胀deal覆盖率"]]
        st.bar_chart(chart_df, use_container_width=True)

    excel_bytes = _make_excel_bytes(city_report, bd_report)
    st.download_button("下载神会员监控Excel", excel_bytes, "神会员监控日报.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
